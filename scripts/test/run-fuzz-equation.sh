#!/usr/bin/env bash
# scripts/test/run-fuzz-equation.sh
#
# Fuzz the equation/formula parser. This drives a
# high-yield robustness surface - parseEquation() tokenises a stored formula
# string, the text a user types and, more importantly, the text restored from an
# imported calculator state (allFormulae round-trips through save/restore).
#
# It overlays the libFuzzer harness (scripts/test/tooling/fuzz-equation.patch,
# carried off the test/fuzz-equation-harness branch off upstream master), builds
# the fuzzEquation target under clang with -fsanitize=fuzzer,address,undefined
# (ASan is the crash gate; UBSan runs in report mode; the alignment check is off
# because the calculator packs data unaligned by design), and runs a time-boxed
# campaign over the seed corpus with the equation-token dictionary.
#
...
TOOLING_PATCH="${TOOLING_PATCH:-$SCRIPT_DIR/tooling/fuzz-equation.patch}"
SEED_DIR="${SEED_DIR:-$SCRIPT_DIR/tooling/fuzz-equation-seeds}"
DICT="${DICT:-$SCRIPT_DIR/tooling/fuzz-equation.dict}"
BUILD_DIR="${BUILD_DIR:-build.fuzzeq}"
...
main() {
    ...
    harness_log "fuzzing parseEquation against upstream $commit"
    ...
    if ! git -C "$UPSTREAM_DIR" apply --check "$TOOLING_PATCH" 2> /dev/null; then
        harness_die "fuzz-equation.patch does not apply on upstream $commit; rebase the
        test/fuzz-equation-harness tooling onto current upstream and regenerate
        scripts/test/tooling/fuzz-equation.patch"
    fi
    ...
    harness_log "building fuzzEquation under clang ($BUILD_DIR)"
    (
        cd "$UPSTREAM_DIR"
        ...
        ninja -C "$BUILD_DIR" "-j$(harness_jobs)" src/testSuite/fuzzEquation >> "$LOG_DIR/fuzz-build.log" 2>&1
    ) || harness_die "fuzz build failed (see $LOG_DIR/fuzz-build.log)"

    local bin="$UPSTREAM_DIR/$BUILD_DIR/src/testSuite/fuzzEquation"
    [[ -x "$bin" ]] || harness_die "fuzzEquation binary not built"
    ...
    if [[ -n "$crashes" ]]; then
        harness_log "FUZZ FINDING(s) - reproducers saved (re-run: fuzzEquation <file>):"
        printf ' %s\n' $crashes
